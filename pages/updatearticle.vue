        ...
        <div class="box-container">
            ...
            <el-row class="list-container">
                ...
                <el-col :span="21" class="cascader-container">
                    <el-cascader-panel :options="options" @change="handleTypeChange" v-model="typeResult"></el-cascader-panel>
                </el-col>
            </el-row>
...
export default {
    ...
    methods: {
        ...
        async submit() {
            ...
            if (this.title && this.typeResult.length && this.editorContent) {
                const {
                    status,
                    data: { code, msg }
                } = await this.$axios.post("http://kingmusi.cn/blog/update", {
                    id,
                    title: that.title,
        ...
        async getContent(){
            const that = this
            const id = this.$route.query.id
            const { status, data:{code, title, content, img, bigType, smallType, msg}} = await this.$axios.post('http://kingmusi.cn/blog/getBlog', {
                id
            })
            ...
            if(status === 200 && code === 0){
                ...
                this.fileList =img ? [{name:'1.jpg', url:img}] : [],
                this.typeResult.push(bigType)
                this.typeResult.push(smallType)
            }
        }
    },
    async mounted() {
